...
SID sid;
bool lockN64Updates = false;

void i2cRequest(){
    lockN64Updates = true;
    Wire.write(N64_transfer_data, 4);
    lockN64Updates = false;
}
...
void setup()
{
    ...
    for (x=0; x<64; x++) {
        ...
        // be plenty.SPCR |= bit (SPE);
    
        // configura o pino MISO como saida
        pinMode(MISO, OUTPUT);
    
        // liga as interrupcoes
        // attachInterrupt(digitalPinToInterrupt(10), onReceive, FALLING);
...
void translate_raw_data()
{
    ...
    N64_transfer_data[0] = N64_status.data1;
    N64_transfer_data[1] = N64_status.data2;
    N64_transfer_data[2] = N64_status.stick_x;
    N64_transfer_data[3] = N64_status.stick_y;
}
...
int sidRegFrameCount[25];
static int sidRegPtrCount[25];
static int sidRegVal[25];

void loop()
{
    digitalWrite(BUSY_PIN, HIGH);
    // for(uint8_t sidRegister=0;sidRegister<=24;sidRegister++){
    //     sidRegFrameCount[sidRegister] = 0;
    //     sidRegPtrCount[sidRegister] = 1;
    //     sidRegVal[sidRegister] = 0;
    // }

    // int x = 0;
    // for(uint16_t sidPointer=0;sidPointer<=totalFrames;sidPointer++){
    //     for(int sidRegister=0;sidRegister<=24;sidRegister++){
    //         sidRegFrameCount[sidRegister]--;
    //         if(sidRegFrameCount[sidRegister] <= 0){
    //             const RegisterData* regPtr = (const RegisterData*) pgm_read_ptr(&rdG[sidRegister]);
    //             uint32_t regData = (uint32_t) pgm_read_dword(&regPtr[sidRegPtrCount[sidRegister]++]);
    //             uint8_t value = regData & 0xFF;
    //             uint16_t frameCounter = (regData & 0xFFFF00) >> 8;
    //             sidRegFrameCount[sidRegister] = frameCounter;
    //             sidRegVal[sidRegister] = value;
    //         }
    //         sid.set_register(sidRegister, sidRegVal[sidRegister]);
    //         // Serial.print(sidRegister);
    //         // Serial.print(" ");
    //         // Serial.print(sidRegFrameCount[sidRegister]);
    //         // Serial.print(" ");
    //         // Serial.print(sidRegVal[sidRegister]);
    //         // Serial.print("|");


    //         // const RegisterData* regPtr = (const RegisterData*) pgm_read_ptr(&rdG[sidRegister]);
    //         // uint16_t regData = (uint16_t) pgm_read_word(&regPtr[sidRegPtrCount[sidRegister]]);
    //         // uint8_t value = regData & 0xFF;
    //         // uint8_t frameCounter = (regData & 0xFF00) >> 8;

    //         // Serial.println(value);
    //         // Serial.println(frameCounter);
    //         // Serial.println(sidRegister);
    //         // Serial.println(sidRegPtrCount[sidRegister]);
    //         // Serial.println((int8_t)(rdG[sidRegister][sidRegPtrCount[sidRegister]].val));
    //         // Serial.println(rdG[sidRegister][sidRegPtrCount[sidRegister]++].frameCount);
    //         // Serial.println(sidRegPtrCount[sidRegister]);
    //         // Serial.println(sidRegister);
    //         // Serial.println(sidRegFrameCount[sidRegister]);
    //         // Serial.println((int8_t)rdG[sidRegister][sidRegPtrCount[sidRegister]].val);
